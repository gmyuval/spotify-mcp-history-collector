...
Edit the external production file on the server (one email per line). Do not
store real addresses in the Git checkout or copy the example template into
production unchanged:

```bash
sudoedit /opt/spotify-mcp-config/authenticated-emails.txt
sudo chown deploy:deploy /opt/spotify-mcp-config/authenticated-emails.txt
sudo chmod 0644 /opt/spotify-mcp-config/authenticated-emails.txt
test -r /opt/spotify-mcp-config/authenticated-emails.txt
test -s /opt/spotify-mcp-config/authenticated-emails.txt
```

The `deploy:deploy` parent directory is mode `0750`. The file is mode `0644`
because oauth2-proxy runs as a different non-root UID; the restricted parent
prevents unrelated users from traversing to it. Compose mounts the file
read-only.
...
2. Edit `/opt/spotify-mcp-config/authenticated-emails.txt`
...
1. Remove their email from `/opt/spotify-mcp-config/authenticated-emails.txt`
...
Your email is not in `/opt/spotify-mcp-config/authenticated-emails.txt`. Add it
and restart oauth2-proxy.
