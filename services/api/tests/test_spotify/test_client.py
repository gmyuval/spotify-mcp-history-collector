...
def _playlist_tracks_page(
    track_ids: list[str],
    total: int,
    next_url: str | None = None,
) -> dict[str, Any]:
    """Helper to build a playlist tracks paging response."""
    return {
        "items": [
            {
                "track": {"id": tid, "name": f"Track {tid}", "artists": [{"id": "a1", "name": "Artist"}]},
...
async def test_get_playlist_all_tracks_single_page() -> None:
    ...
    client = SpotifyClient("test-token", max_retries=0)
    items = await client.get_playlist_all_tracks("pl1")
    assert len(items) == 3
    assert items[0].track is not None
    assert items[0].track.id == "t1"
    assert items[2].track is not None
    assert items[2].track.id == "t3"
...
async def test_get_playlist_all_tracks_multi_page() -> None:
    ...
    # initial request and the `next` URL hit the same route.
    responses = iter(
        [
            httpx.Response(200, json=_playlist_tracks_page(["t1", "t2"], total=3, next_url=page2_url)),
    ...
    client = SpotifyClient("test-token", max_retries=0)
    items = await client.get_playlist_all_tracks("pl1", page_size=2)
    assert len(items) == 3
    assert [it.track.id for it in items if it.track] == ["t1", "t2", "t3"]
...
async def test_get_playlist_all_tracks_respects_max_tracks() -> None:
    ...
    # Page 2 should NOT be fetched because max_tracks=3 is already met
    page2_route = respx.get(page2_url).mock(
        return_value=httpx.Response(
            200,
            json=_playlist_tracks_page(["t4", "t5"], total=100),
        )
    )
...
# ---------------------------------------------------------------------------
# Playlist metadata-endpoint pagination (403 fallback)
# ---------------------------------------------------------------------------


def _full_playlist_response(
    playlist_id: str,
    track_ids: list[str],
    total: int,
) -> dict[str, Any]:
    """Helper to build a full playlist response with embedded tracks."""
    return {
        "id": playlist_id,
        "name": "Test Playlist",
        "tracks": {
...
@respx.mock
async def test_get_playlist_all_tracks_via_metadata_single_page() -> None:
    """Metadata pagination with all tracks in one page."""
    respx.get("https://api.spotify.com/v1/playlists/pl1").mock(
        return_value=httpx.Response(
            200,
            json=_full_playlist_response("pl1", ["t1", "t2", "t3"], total=3),
        )
    )

    client = SpotifyClient("test-token", max_retries=0)
    items = await client.get_playlist_all_tracks_via_metadata("pl1")
    assert len(items) == 3
    assert [it.track.id for it in items if it.track] == ["t1", "t2", "t3"]


@respx.mock
async def test_get_playlist_all_tracks_via_metadata_multi_page() -> None:
    """Metadata pagination fetches all tracks across multiple pages."""
    total = 180
    page_size = 50
    track_ids = [f"t{i}" for i in range(total)]
    ...
    for offset in range(0, total, page_size):
        ...
        pages.append(
            httpx.Response(
                200,
                json=_full_playlist_response("pl1", chunk, total=total),
            )
        )
    ...
    client = SpotifyClient("test-token", max_retries=0)
    items = await client.get_playlist_all_tracks_via_metadata("pl1", page_size=page_size)
    assert len(items) == 180
    assert items[0].track is not None
    assert items[0].track.id == "t0"
    assert items[179].track is not None
